...
function convertDetailLevelLinks() {
    ...
    if (detailLevels) {
        ...
        for (var i = 1; i <= spanLinks.length; i++) {
            ...
            hyperlink.innerHTML = i.toString();
        ...
        for (var j = 0; j < oldRows.length; j++) {
            ...
            for (var k = 0; k < oldCols.length; k++) {
                ...
                if (newCol.classList.contains("desc")) {
                    ...
                    newCol.classList.add("s" + (6 - width / 16));
                    // append an indenting column
                    if (width) {
                        var indent = document.createElement("div");
                        indent.classList.add("col", "s" + (width / 16));
...
/**
 * Overwrite the toggleLevel function provided by Doxygen as we have altered the DOM.
 * @param {number} level the The tree level to toggle.
 */
...
/**
 * Overwrite the toggleFolder function generated by doxygen as we have altered the DOM.
 * @param {string} id The suffix for the element's ID that pertain to the specified folder.
 */
